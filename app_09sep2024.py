...
def load_user(user_id):
    try:
        ...
        if user:
            # Do not modify the session here. Just return the user.
            return user
    except Exception as e:
        logging.error(f"Error loading user with ID {user_id}: {str(e)}")
    return None
...
with app.app_context():
    ...
    print('Team blueprint registered')
...
def inject_user_roles():
    # Get user roles from the session or default to ['Guest']
    user_roles = session.get('user_roles', ['Guest'])

    # Safely check if the user is authenticated using Flask-Login's attribute
    is_authenticated = current_user.is_authenticated if current_user.is_authenticated else False

    # Debugging output to verify roles and authentication status
    print(f"Injected roles: {user_roles}, Authenticated: {is_authenticated}")

    return dict(user_roles=user_roles, is_authenticated=is_authenticated)

@app.before_first_request
def clear_session_on_startup():
    print("Clearing session on first request.")
    session.clear()  # Clear session only once when the app starts
...
# parsed_menu_data = menu_builder.parse_menu_data(user_roles=["Guest"], is_authenticated=False)
...
def generate_route_and_menu(route, allowed_roles, template, limited_menu=False):
    def decorator(func):
        @app.route(route)
        @wraps(func)
        def wrapper(*args, **kwargs):
            ...
            # Access the user roles and authentication status from the context
            user_roles = g.get('user_roles', ['Guest'])  # Use 'g' or directly from context in templates
...
def get_menu_builder():
    # Lazy initialization of MenuBuilder
    if not hasattr(g, 'menu_builder'):
        # Initialize only when accessed
        g.menu_builder = MenuBuilder(main_menu_items, ["Guest"])
        print('MenuBuilder initialized')
...
def login():
    ...
    if form.validate_on_submit() and request.method == 'POST':
        ...
        print('User Captcha:', user_captcha)
        if 'captcha' in session and session['captcha'] == user_captcha:
            print('User Captcha matched.')
            try:
                ...
                if user:
                    ...
                    # Set user roles in the session
                    # Correct role assignment
                    user_roles = [role.name for role in user.roles] if user.roles else ['Guest']
                    session['user_roles'] = user_roles
                    session['user_id'] = user.id
                    session['username'] = username
                    session['email'] = user.email
                    session.modified = True  # Force Flask to save the session changes
                    print(f"Roles for user {username}: {user_roles}")

                    print(f"Session after login: {session}")

                    # Login the user if not already authenticated
                    if not current_user.is_authenticated:
                        remember = 'remember' in request.form  # User-defined setup
                        ...
                        try:
                            create_message(
                                db.session, user_id=user.id, message_type='email', subject='Security check',
                    ...
                    # Create a secure response with the appropriate cookie attributes
                    response = make_response(redirect_based_on_role(user))
                    response.set_cookie('session', value=session.sid, httponly=True, secure=True, samesite='Lax')
                    return response
...
def logout():
    try:
        ...
        # Clear all session data to ensure no sensitive information is kept
        session.clear()
