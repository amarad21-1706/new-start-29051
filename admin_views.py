...
class PhysicalSettlementAdminView(BaseView):
    def __init__(self, session, **kwargs):
        super().__init__(**kwargs)
        self.session = session
...
class Tabella22_dataView(ModelView):
    ...
    # Specify the fields to be edited inline using XEditableWidget
    column_editable_list = ['subject_id', 'interval_ord', 'fi0', 'fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fn1', 'fn2', 'fc1']
    # Customize the widget for inline editing
    form_widget_args = {
        # 'fi0': {'widget': XEditableWidget()},
        # 'interval_ord': {'widget': XEditableWidget()},
    ...
    column_list = ('subject_id', 'company_id', 'fi0', 'interval_ord', 'fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fn1', 'fn2', 'fc1')
    form_columns = ('subject_id', 'fi0', 'interval_ord', 'fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fn1', 'fn2', 'fc1')  # Specify form columns with dropdowns

    column_labels = {'subject_id': 'Fascia di domanda',
                     'company_id': 'Comp.',
                     'interval_ord': 'Periodo', 'fi0': 'Anno',
                     'fi1': 'UDD *', 'fi2': 'PdR total *',
                     'fi3': 'PdR Switch *',
                     'fi4': 'Quantità (Smc/KWh) *',
                     'fi5': 'Quantità Switch (Smc/KWh) *',
                     'fn1': '% PdR Switch', 'fn2': '% Smc/KWh switch',
                     'fc1': 'Note'}
    column_descriptions = {'subject_id': 'Fascia di domanda',
                           'company_id': 'Comp',
                           ...
                           'fi2': 'Numero PdR (b)',
                           'fi3': 'PdR Switch (c)',
                           'fi4': 'Quantità (Smc/KWh) (d)',
                           'fi5': 'Quantità Switch (Smc/KWh) (e)',
                           'fn1': '% PdR Switch (f=c/b)',
                           'fn2': '% Quantità switch (g=e/d)',
                           'fc1': 'Note (opzionale)'}

    # Customize inlist for the View class
    column_searchable_list = ('fi0', 'fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fc1')  # Adjust based on your model structure
    column_filters = ('fi0', 'fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fc1')  # Adjust based on your model structure

    # Specify fields to be excluded from the form
    form_excluded_columns = ('company_id', 'user_id', 'status_id', 'created_by')
...
class Tabella22_dataView_OLD(ModelView):
    create_template = 'admin/create_base_data.html'
    area_id = 2
    subarea_id = 10  # Define subarea_id as a class attribute
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        # self.class_name = self.__class__.__name__  # Store the class name
        self.subarea_id = Tabella22_dataView_OLD.subarea_id  # Initialize subarea_id in __init__
        self.area_id = Tabella22_dataView_OLD.area_id  # Initialize area_id in __init__
        self.subarea_name = get_subarea_name(area_id=self.area_id, subarea_id=self.subarea_id)
    ...
    def scaffold_form(self):
        form_class = super(Tabella22_dataView_OLD, self).scaffold_form()
    ...
    def create_model(self, form):
        model = super(Tabella22_dataView_OLD, self).create_model(form)
        if current_user.is_authenticated:
            try:
                model.user_id = current_user.id  # Set the user_id
                model.company_id = session['company_id']  # Set the company_id
    ...
    def get_query(self):
        query = super(Tabella22_dataView_OLD, self).get_query().filter_by(data_type=self.subarea_name)
    ...
    def on_model_change(self, form, model, is_created):
        super(Tabella22_dataView_OLD, self).on_model_change(form, model, is_created)
        # Reset form data
        form.populate_obj(model)  # This resets the form data to its default values
...
def create_admin_views(app, intervals):
    ...
    with app.app_context():
        ...
        class CustomFlussiDataView(ModelView):
            ...
            column_list = ['company_id', 'interval_ord', 'fi0', 'fi1', 'fi2', 'fi3', 'fc1']
            ...
            def scaffold_form(self):
                ...
                form_class.subcategory_id = QuerySelectField(
                    'Categoria',
                    query_factory=self.get_filtered_subcategories,
                    allow_blank=True,
                    get_label='name',
                    validators=[Optional()],
                    widget=Select2Widget()
                ...
                form_class.item_id = QuerySelectField(
                    'Articolo',
                    query_factory=self.get_filtered_items,
                    allow_blank=True,
                    get_label='name',
                    validators=[Optional()],
                    widget=Select2Widget()
            ...
            def get_filtered_subcategories(self):
                lexic_id = self._current_form_data.get('lexic_id')
                print("Filtering subcategories with lexic_id:", lexic_id)  # Debug
                if lexic_id:
                    subcategories = LexicSubcategory.query.filter_by(lexic_id=lexic_id).all()
                    print("Subcategories found:", subcategories)  # Debug
                    return subcategories
                return []

            def get_filtered_items(self):
                subcategory_id = self._current_form_data.get('subcategory_id') if self._current_form_data else None
                print("Filtering items with subcategory_id:", subcategory_id)  # Debug
                if subcategory_id:
            ...
            def create_form(self, obj=None):
                """Ensure form data is cached for filtering."""
                self._current_form_data = {}  # Reset cache
                form = super().create_form(obj)
                # Cache form data after form initialization

                self._current_form_data = form.data

                # Debug form data
                print("Create form data cached:", self._current_form_data)

                # Set default values for the dropdowns if needed
                form.lexic_id.data = form.lexic_id.data or None
                form.subcategory_id.data = form.subcategory_id.data or None
                form.item_id.data = form.item_id.data or None
                # Debugging form data
                print("Create form data cached:", self._current_form_data)
                print("Lexic ID in form:", form.lexic_id.data)
                print("Subcategory ID in form:", form.subcategory_id.data)
                print("Item ID in form:", form.item_id.data)
                self._current_form_data = form.data
                print("Create form data cached:", self._current_form_data)  # Debug
                return form
            ...
            def edit_form(self, obj=None):
                ...
                # Debug form data
                print("Edit form data cached:", self._current_form_data)

                self._current_form_data = form.data
                print("Edit form data cached:", self._current_form_data)  # Debug
                return form
            ...
            def create_model(self, form):
                try:
                    ...
                    # Link selected subcategory and item
                    subcategory_id = form.subcategory_id.data
                    item_id = form.item_id.data

                    if subcategory_id:
                        subcategory = LexicSubcategory.query.get(subcategory_id)
                        if not subcategory:
                            raise ValidationError("Invalid Subcategory selected.")

                    if item_id:
                        item = LexicItem.query.get(item_id)
                        if not item:
            ...
            def on_model_change(self, form, model, is_created):
                ...
                if not form.lexic_id.data:
                    raise ValidationError('Tipo pre-complaint is required.')
                if form.subcategory_id.data and form.subcategory_id.data.lexic_id != form.lexic_id.data.id:
                    raise ValidationError('Selected Categoria does not belong to the selected Tipo pre-complaint.')
                if form.item_id.data and form.item_id.data.subcategory_id != form.subcategory_id.data.id:
                    raise ValidationError('Selected Articolo does not belong to the selected Categoria.')
        ...
        class AttiDataView(BaseDataView):
            ...
            # Adjusted order of fields
            column_list = (
            'company_id', 'number_of_doc', 'date_of_doc', 'file_path', 'no_action', 'subject_id', 'fi0', 'interval_ord',
            'fc2')
            form_columns = (
            'number_of_doc', 'date_of_doc', 'file_path', 'no_action', 'subject_id', 'fi0', 'interval_ord', 'fc2')
            ...
            def on_model_change(self, form, model, is_created):
                ...
                if existing_document and existing_document.id != model.id:
                    raise ValidationError(
                        'A document with the same number, date, subarea, area, and subject already exists.')
        ...
        admin_app1.add_view(
            PhysicalSettlementAdminView(session=db.session, name='Settlement fisico', endpoint='physical_settlement_within_admin'))
