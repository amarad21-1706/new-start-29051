...
from flask import session, send_file, render_template, flash, redirect, url_for, request, current_app, get_flashed_messages, g
...
from config.config import initialize_intervals_and_configs
from flask import Flask
from app.modules.db import db

# from userManager101 import UserManager
# from workflow_manager import (create_card,
...
from models.user import (Users, UserRoles, Role, Table, Questionnaire, Question,
                         ...
                         Answer, Company, QuestionnaireCompanies, CompanyUsers, Lexic,
                         Interval, Subject,
                         AuditLog, Post, Workflow, Step, BaseData, WorkflowSteps, WorkflowBaseData,
...
from forms.forms import (ForgotPasswordForm, ResetPasswordForm101, RegistrationForm,
                         QuestionnaireCompanyForm, QuestionnaireQuestionForm, WorkflowStepForm, WorkflowBaseDataForm,
                         BaseDataWorkflowStepForm,
                         UserRoleForm, CompanyUserForm, UserDocumentsForm, create_dynamic_form, CustomFileLoaderForm,
...
'''
from app.utils.utils import get_current_directory, get_current_intervals, get_areas, get_subareas, get_except_fields
from config.config import get_current_interval
...
# Import routes and blueprints within the function or method
def initialize_app():
    app = create_app()
    ...
    with app.app_context():
        ...
        # Initialize intervals and other configurations
        initialize_intervals_and_configs(db)

        # Import routes and blueprints within the function or method
        from routes import routes  # Ensure routes is imported after app is initialized
        from userManager101 import UserManager  # Import here to avoid circular import
        from workflow_manager import (create_card,
                                      get_model_statistics, create_deadline_card,
                                      deadline_approaching)
        from app_defs import get_user_roles, create_message, generate_menu_tree
        ...
        from forms.forms import (ForgotPasswordForm, ResetPasswordForm101, RegistrationForm,
                                 QuestionnaireCompanyForm, QuestionnaireQuestionForm, WorkflowStepForm,
                                 WorkflowBaseDataForm,
                                 BaseDataWorkflowStepForm,
                                 UserRoleForm, CompanyUserForm, UserDocumentsForm, create_dynamic_form,
                                 CustomFileLoaderForm,
                                 CustomSubjectAjaxLoader, BaseSurveyForm)

        from config.config import (get_current_interval, get_current_intervals,
                                   get_subarea_interval_type, generate_company_questionnaire_report_data,
                                   generate_area_subarea_report_data,
                                   check_status, check_status_limited, check_status_extended, generate_html_cards,
                                   get_session_workflows,
                                   generate_html_cards_progression_with_progress_bars111,
                                   generate_html_cards_progression_with_progress_bars_in_short,
                                   get_subarea_name, get_pd_report_from_base_data_wtq, get_areas, create_notification,
        ...
        from menu_builder import MenuBuilder
        user_manager = UserManager()  # Now you can use UserManager
        # user_manager.manage_users() # TODO meaning?

        from app.utils.utils import get_current_directory  # Correct import statement
        print(get_current_directory())

        from app.modules.custom_admin_views import create_custom_admin_views
        create_custom_admin_views(app)

        # crud_bp = create_crud_blueprint()
...
@app.route('/open_admin_app_1222')
def open_admin_app_1222():
    logging.debug("Admin 1 index accessed")
    # Check if the route redirects to itself or another route that might cause a loop
    current_app.logger.debug("Admin 1 index accessed")
...
def open_admin_app_1():
    try:
        ...
        # Check if the target route is different to avoid circular redirect
        target_route = 'open_admin.index'
        if request.endpoint == target_route:
            raise Exception("Circular redirect detected")

        return redirect(url_for(target_route))
    except Exception as e:
        current_app.logger.error(f"Error in open_admin_app_1: {str(e)}")
        return render_template('error.html', error_message=str(e)), 500

@app.route('/open_admin')
def open_admin():
    try:
        current_app.logger.debug("Handling request for /open_admin/")
        # Ensure this route does not cause a circular redirect
        return "Admin Page"
    except Exception as e:
        current_app.logger.error(f"Error in open_admin: {str(e)}")
        return render_template('error.html', error_message=str(e)), 500
